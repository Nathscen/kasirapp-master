...
function EditProductPetugas() {
    ...
    const fetchProduct = async () => {
      try {
        ...
        const response = await axios.get(
          `http://127.0.0.1:8080/petugas/get_produk/${productId}`,
          {
            headers: {
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      ...
      const token = localStorage.getItem("token");
      await axios.patch(
        `http://127.0.0.1:8080/petugas/edit_produk/${productId}`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
...
export default EditProductPetugas;
